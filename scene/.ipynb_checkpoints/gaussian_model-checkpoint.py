...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, opt):
        ...
        self._sky_model = SkyCubeMap(opt)
        self._deformation_table = torch.empty(0)
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        embedding = torch.zeros((fused_color.shape[0], self._deformation.gaussian_embedding_dim)).float().cuda()
    ...
    def create_from_anchor(self, path, spatial_lr_scale):
        print("正在从anchor中初始化3DGS")
        self.spatial_lr_scale = spatial_lr_scale
    ...
    def densification_postfix(self, new_deformation_table, new_embedding, new_xyz, new_features_dc, new_features_rest,
                              new_opacities,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_deformation_table, new_embedding, new_xyz, new_features_dc, new_features_rest,
                                   new_opacity, new_scaling,
                                   new_rotation)
    ...
    def densify_from_depth_propagation(self, viewpoint_cam, propagated_depth, filter_mask, gt_image):
        ...
        coordinates = torch.stack(
            [x.to(propagated_depth.device), y.to(propagated_depth.device), torch.ones_like(propagated_depth)], dim=-1)
        ...
        new_dynamic = nn.Parameter(torch.zeros((fused_point_cloud.shape[0], 1), device="cuda").requires_grad_(True))
        ...
        n_embedding = torch.zeros(
            (fused_point_cloud.shape[0], self._deformation.gaussian_embedding_dim)).float().cuda()  # [jm]
        new_embedding = nn.Parameter(torch.zeros((fused_point_cloud.shape[0], self._deformation.gaussian_embedding_dim),
                                                 device="cuda").requires_grad_(True))
        # update gaussians
        new_embedding = nn.Parameter(n_embedding.requires_grad_(True))
        new_deformation_table = torch.gt(torch.ones((fused_point_cloud.shape[0]), device="cuda"), 0)
        self.densification_postfix(new_deformation_table, new_embedding, new_xyz, new_features_dc, new_features_rest,

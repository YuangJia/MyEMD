...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, grid_pe=0, skips=[], args=None):
        ...
        if args.zero_temporal:
            ...
            self.weight = torch.nn.Parameter(torch.normal(0., 0.01 / np.sqrt(self.temporal_embedding_dim),
                                                          size=(self.max_embeddings, self.temporal_embedding_dim)))
        ...
        if not self.args.no_time_offset:
            self.time_offset = torch.nn.Parameter(
                torch.zeros((3, 1)))  # hard coded the upper limit of the num cameras (adjust as necessary)
    ...
    def create_net(self):
        ...
        if not self.args.no_fine_deform:
            ...
            self.rotations_deform_f = nn.Sequential(nn.ReLU(), nn.Linear(self.W, self.W), nn.ReLU(),
                                                    nn.Linear(self.W, 4))
            self.opacity_deform_f = nn.Sequential(nn.ReLU(), nn.Linear(self.W, self.W), nn.ReLU(), nn.Linear(self.W, 1))
            self.shs_deform_f = nn.Sequential(nn.ReLU(), nn.Linear(self.W, self.W), nn.ReLU(),
    ...
    def get_temporal_embed(self, t, current_num_embeddings, align_corners=True):
        ...
        grid = \
        torch.cat([torch.arange(fdim).cuda().unsqueeze(-1) / (fdim - 1), torch.ones(fdim, 1).cuda() * t, ], dim=-1)[
            None, None, ...]
        grid = (grid - 0.5) * 2
        ...
        if not self.args.no_temporal_embedding_dim:
            ...
            if use_coarse_temporal_embedding:
                ...
                if self.args.no_c2f_temporal_embedding:
                    ...
                    h = self.get_temporal_embed(t, self.int_lininterp(iter, num_down_emb, self.max_embeddings,
    ...
    def get_feature(self, rays_pts_emb, time_emb, embeddings, \
        ...
        hidden = self.query_time(hidden_hexplane, time_emb, embeddings, \
                                 iter=iter, feature_out=feature_out,
                                 use_coarse_temporal_embedding=use_coarse_temporal_embedding, num_down_emb=num_down_emb)
        ...
        if self.args.aggregate_feature:
            ...
            hidden_forward = self.query_time(hidden_hexplane_forward, forward_warped_time_emb, embeddings, \
                                             iter=iter, feature_out=feature_out,
                                             use_coarse_temporal_embedding=use_coarse_temporal_embedding,
                                             num_down_emb=num_down_emb)
            ...
            if apply_deform:
                if is_coarse:
                    hidden = self.get_feature(rays_pts_emb, time_emb, embeddings, \
                                              iter, self.feature_out, use_coarse_temporal_embedding=True,
                                              num_down_emb=num_down_emb_c, time_diff=time_diff, is_train=is_train)
                    ddict = self.forward_dynamic(hidden, \
                                                 self.pos_deform, self.scales_deform, self.rotations_deform,
                                                 self.opacity_deform, self.shs_deform)
                else:
                    hidden = self.get_feature(rays_pts_emb, time_emb, embeddings, \
                                              iter, self.feature_out_f, use_coarse_temporal_embedding=False,
                                              num_down_emb=num_down_emb_f, time_diff=time_diff, is_train=is_train)
                    ddict = self.forward_dynamic(hidden, \
                                                 self.pos_deform_f, self.scales_deform_f, self.rotations_deform_f,
                                                 self.opacity_deform_f, self.shs_deform_f)
            else:
                # pts = rays_pts_emb[:,:3]
                # scales = scales_emb[:,:3]
...
class deform_network(nn.Module):
    ...
    def apply_deform(self, point, scales=None, rotations=None, opacity=None, shs=None, ddict_c=None, ddict_f=None):
        ...
        rotations_final = rotations.clone() if self.args.direct_add_dr or self.args.no_dr else torch.zeros_like(
            rotations)
    ...
    def forward(self, point, scales=None, rotations=None, opacity=None, shs=None, times_sel=None, \
        ...
        ddict_c = self.deformation_net(point_emb,
                                       # scales_emb,
                                       # rotations_emb,
                                       # opacity,
                                       # shs,
        ...
        point_final, scales_final, rotations_final, opacity_final, shs_final = self.apply_deform(point, scales,
                                                                                                 rotations, opacity,
                                                                                                 shs, ddict_c, ddict_f)

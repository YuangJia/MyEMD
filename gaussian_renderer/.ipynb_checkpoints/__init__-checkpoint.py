...
def render(args, viewpoint_camera, pc: GaussianModel, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None, \
    ...
    if "fine" in stage:
        ...
        if args.combine_dynamic_static:
            ...
            if colors_precomp_final is not None:
                ...
                shs_final = shs_final.view(shs_final.shape[0], -1) * dynamic_ratio + shs.view(shs.shape[0],
                                                                                              -1) * static_ratio
    ...
    if return_decomposition and stage == "fine":
        ...
        def render_dx(dx, do, is_static=True):
            ...
            if args.combine_dynamic_static:
                ...
                rendered_image_d, depth_d, normal_d, weight_d, radii_d, _ = rasterizer(
                    ...
                    colors_precomp=colors_precomp_final[dynamic_mask] if colors_precomp_final is not None else None,
                    # [N,3]
        ...
        if not args.no_coarse_deform:
            result_dict['ddict_render']['coarse_render'] = render_dx(ddict['coarse']['dx'], ddict['coarse']['do'],
                                                                     is_static=False)

        if not args.no_fine_deform:
            result_dict['ddict_render']['fine_render'] = render_dx(ddict['fine']['dx'], ddict['fine']['do'],
                                                                   is_static=True)

        if not args.no_coarse_deform and not args.no_fine_deform:
            result_dict['ddict_render']['coarse_fine_render'] = render_dx(ddict['coarse']['dx'] - ddict['fine']['dx'],
                                                                          ddict['coarse']['do'] - ddict['coarse']['do'])

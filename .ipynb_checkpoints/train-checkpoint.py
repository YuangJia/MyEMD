...
from utils.graphics_utils import surface_normal_from_depth, img_warping, depth_propagation, check_geometric_consistency, \
    generate_edge_mask
import sys
from scene import Scene, GaussianModel, EUVSScene
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,
             reference_begin_iter, reference_end_iter, tb_writer, gaussians, scene, stage="coarse",
             lpips_criteria=None):
    first_iter = reference_begin_iter
    ...
    progress_bar = tqdm(range(first_iter, reference_end_iter), desc="Training progress")
    first_iter += 1
    ...
    propagation_dict = {}
    for i in range(0, scene.getTrainCameraNum(), 1):
        viewpoint_cam = scene.getTrainCameraById(i)
    ...
    for iteration in range(first_iter, reference_end_iter + 1):
        ...
        while network_gui.conn != None:
            try:
                ...
                if do_training and ((iteration < int(reference_end_iter)) or not keep_alive):
                    break
        ...
        randidx = randint(0, scene.getTrainCameraNum() - 1)
        ...
        viewpoint_cam = scene.getTrainCameraById(randidx)
        ...
        if 'fine' in stage:
            ...
            if args.time_smoothness_weight != 0:
                # tv_loss = 0
                tv_loss = gaussians.compute_regulation(args.time_smoothness_weight, args.l1_time_planes,
                                                       args.plane_tv_weight)
                loss += tv_loss
        ...
        with torch.no_grad():
            ...
            if iteration == reference_end_iter:
                progress_bar.close()

            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                            testing_iterations, scene, render, (pipe, background), opt)
            # if (iteration in saving_iterations) or (stage == "fine" and iteration == reference_end_iter):
            if (iteration in saving_iterations) or ("fine" in stage and iteration == reference_end_iter):
...
def main_train(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint,
               debug_from):
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree, opt)
    scene = EUVSScene(dataset, gaussians)
    lpips_criteria = None
    if dataset.use_lpips_loss:#vgg alex
        print("Using LPIPS Loss!")
        lpips_criteria = lpips.LPIPS(net='vgg').cuda()
    if "level3" in dataset.source_path:
        middle = 18000
        final = 45000
    else:
        middle = 10000
    ...
    # training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, 0, 40000, tb_writer, gaussians, scene, stage="coarse")
    # train_dynamic(pipe,opt,scene,gaussians,tb_writer)
    # training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, 15000, 35000, tb_writer, gaussians, scene, stage="fine")
...
if __name__ == "__main__":
    ...
    main_train(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations,
